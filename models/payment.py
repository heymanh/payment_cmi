...
class PaymentAcquirerCmi(models.Model):
    ...
    cmi_tx_confirmation = fields.Boolean(string='Automatic confirmation mode', default=True, help="Check this box to confirm CMI transactions automatically.")
    ...
    def cmi_form_generate_values(self, values):
        ...
        cmi_values = dict(values,
                                ...
                                BillToCompany=re.sub(r'[^a-zA-Z0-9 ]+', '', values.get('billing_partner_commercial_company_name')).strip(),
                                BillToCountry=re.sub(r'[^a-zA-Z0-9 ]+', '', values.get('billing_partner_country').name).strip(),
...
class PaymentTransactionCmi(models.Model):
    ...
    def _cmi_form_get_tx_from_data(self, data):
        ...
        if not reference or not shasign:
            raise ValidationError(_('CMI: received data with missing reference (%s) or hash (%s)') % (reference, shasign))
        ...
        if shasign_check.upper() != shasign.upper():
            raise ValidationError(_('CMI: invalid shasign, received %s, computed %s, for data %s') % (shasign, shasign_check, data))
        return transaction
